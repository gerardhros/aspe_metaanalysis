...
# -- step 1. load data ----
  ...
  d2 <- fread('products/250425 covariates metaanalysis no.csv')
  d2[,c('lat','lon') := NULL]
  ...
  d1.other <- as.data.table(readxl::read_xlsx('data/250704 database aspe no.xlsx',sheet='Andere meststoffen'))
  d1 <- rbind(d1.as,d1.other,fill=TRUE)
  d2 <- fread('products/250429 covariates metaanalysis no.csv')
  d2[,c('lat','lon') := NULL]
...
# -- step 2. clean data -----
  ...
  d1 <- d1[!is.na(EF) & ndose > 0 & !is.na(ndose)]
  ...
  d1[grepl('straw|pig|manure|msw|ridg|furr|resid|fym|pig|slurry|manure|compost|urine|cake|org|municipal|cattle|residue|fish|vetc|crop|msw|straw|rapese|excreta',tolower(fert_type)),fert_cat := 'OM']
  d1[grepl('as|ammonium_sulph|ammonium sulph|nh4so4',tolower(fert_type)) & is.na(fert_cat), fert_cat := 'AS']
  d1[fert_cat =='AS' & grepl('potassium|npk starter|controlled release',tolower(fert_type)),fert_cat := NA_character_]
  d1[fert_cat =='AS'& grepl('dmpp|dmpsa|dcd|inhib|formalde|entec|coat|slow|polym|contr|\\+ni|ni$|\\+ ni',tolower(fert_type)), fert_cat := 'ASC']
  d1[fert_cat =='AS'& grepl('u',tolower(fert_type)),fert_cat := 'UM']
  ...
  d1[grepl('an|ammonium nitra|nh4no3|ammoini|calcium nitrate',tolower(fert_type))& is.na(fert_cat), fert_cat := 'AN']
  d1[fert_cat =='AN'& grepl('dmpp|dmpsa|dcd|inhib|formalde|entec|coat',tolower(fert_type)), fert_cat := 'ANC']
  d1[is.na(fert_cat) & grepl('dap|map',tolower(fert_type)),fert_cat := 'DAP-MAP']
  ...
  d1[,tillage := tolower(tillage)]
  d1[is.na(nsplit),nsplit := 1]
  d1[,nsplitc := as.character(nsplit)]
  d1[nsplit>3, nsplitc := '>3']
  d1[is.na(GEnZ) & country %in% c('ireland','germany','usa','france'), GEnZ := 'J']
  ...
  d1[is.na(GEnZ) & grepl('jap',tolower(location)),GEnZ := 'K']
  ...
  d1[is.na(n), n:= 3]
  d1[,no_fert_cv := no_fert_se/no_fert]
  d1[,no_fert_cv_mean := mean(no_fert_cv,na.rm=T)*1.5]
  d1[is.na(no_fert_se),no_fert_se := no_fert_cv_mean * no_fert]
  
  d1[is.na(no_control), no_control := median(d1$no_control,na.rm=T)]
  d1[,no_contr_cv_mean := no_fert_cv_mean]
  d1[is.na(no_control_se),no_control_se := no_contr_cv_mean * as.numeric(no_control)]
  d1[,c('no_fert_cv','no_fert_cv_mean','no_contr_cv_mean') := NULL]
  
  d1[,no_diff := no_fert - no_control]
  d1[,ef_no := fifelse(!is.na(no_diff),no_diff / ndose,EF)]
  d1[,ef_no_se := sqrt(no_fert_se^2+no_control_se^2)/ndose]
  d1[is.na(ef_no_se),ef_no_se := median(d1$ef_no_se,na.rm = T)]

  # set values below zero to almost zero
  d1[ef_no <= 0, ef_no := pmax(0.0001,ef_no)]
  d1[ef_no_se <= 0, ef_no_se := pmax(0.00001,ef_no_se)]
  
  # convert to log scale
  d1[,log_ef := log(ef_no)]

  # select variables for a model
  d2 <- d1[,.(studyid,lat,lon,region,ef_mean = ef_no,ef_sd = ef_no_se,log_ef,
              GEnZ, prec,temp,crop_cat,phtype,
              sand,clay,soc,cec,ph,bd,n,measmethod  ='unknown',
  ...
  fwrite(d2,'products/250423_db_no_final.csv')
  fwrite(d2,'products/250429_db_no_final.csv')
  
# make a model to predict N2O
m1 <-lme(log_ef ~ fert_cat+ndose + ph+clay+soc +I(ph^2) + prec+temp+
                  appc + nsplitc-1, 
...
d3.s <- d3[fert_cat %in% c('AS','ASN','ASC','ASNC'),studyid]
d3.s <- d3[studyid %in% d3.s]

# make a model to predict N2O for AS studies only
m3.s <-lme(log_ef ~ fert_cat+ndose + ph+ soc + ph + I(temp^2)+
           temp-1, 
         random = ~ 1 |studyid, weights = varFunc(~ log(ef_no_se)),  
         data=d3.s,na.action=na.omit)
